...
const rupeesToPaise = (value: string) => Math.max(0, Math.round((Number(value) || 0) * 100));
const paiseToRupeesInput = (value: number) => (Number(value || 0) / 100).toFixed(2).replace(/\.00$/, '');
...
const emptyServerCost = { id: '', name: '', nodeName: '', monthlyCostPaise: '0', ipCostPaise: '0', plannedVmCapacity: '0', includedIpCount: '0', isActive: true };
...
export const BillingOperationsPanel: React.FC = () => {
  ...
  const load = async () => {
    setLoading(true);
    setError(null);
    const sections = ['summary', 'pricing plans', 'invoices', 'billing policy', 'cost bases', 'dedicated-server costs', 'server profitability', 'VM billing assignments', 'suspension actions'] as const;
    const requests = [
    ...
    const [summaryResult, plansResult, invoicesResult, configResult, costBasesResult, serverCostsResult, profitabilityResult, profilesResult, actionsResult] = results;
  ...
  const saveServerCost = async (event: React.FormEvent) => {
    ...
    try {
      await apiClient.upsertBillingServerCost({
        ...
        nodeName: serverCostForm.nodeName,
    ...
    <main className="app-content min-h-full px-4 py-5 sm:px-6 lg:px-8" aria-busy={loading}>
      <div className="mx-auto w-full">
        <header className="ink-block-wrapper !mb-6 flex flex-col gap-5 p-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="section-title !mb-2 !text-[11px] !tracking-[0.14em] !text-[#8b5e00]">Finance operations · INR reporting</p>
          ...
          <button type="button" onClick={() => void load()} disabled={loading} className="btn-primary h-10 shrink-0 disabled:cursor-not-allowed disabled:opacity-50">{loading ? 'Refreshing…' : 'Refresh data'}</button>
        </header>
        ...
        <section className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6" aria-label="Billing KPIs">
          ...
          ].map(({ label, value, icon: Icon, tone }) => <div key={label} className="tile-white !mb-0 !min-h-[118px] !flex-col !items-start !justify-start !gap-3 !p-5"><div className={`flex h-8 w-8 items-center justify-center rounded-md ${tone}`}><Icon size={16} strokeWidth={1.8} aria-hidden="true" /></div><div><p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-[#8a9090]">{label}</p><p className={`mt-2 text-xl font-semibold tabular-nums ${label === 'INR gross profit' && summary && summary.inrGrossProfitPaise < 0 ? 'text-[#8d3028]' : 'text-[#1a1a1a]'}`}>{value}</p></div></div>)}
        </section>
...
const VmBillingRow: React.FC<{ profile: ApiVmBillingProfile; plans: ApiPricingPlan[]; saving: string | null; onSave: (profile: ApiVmBillingProfile, planId: string, customPrice: string, ipCount: string) => Promise<void> }> = ({ profile, plans, saving, onSave }) => {
  ...
  useEffect(() => { setPlanId(profile.planId || ''); setCustomPrice(profile.customMonthlyPriceCents === null ? '' : String(profile.customMonthlyPriceCents)); setIpCount(String(profile.ipCount || 1)); }, [profile.planId, profile.customMonthlyPriceCents, profile.ipCount]);
  return <tr className="text-xs text-[#1a1a1a]"><td className="px-5 py-3"><span className="font-semibold">VM-{profile.vmid} · {profile.vmName || 'Unnamed guest'}</span><span className="mt-1 block text-[10px] text-[#8a9090]">Due {dateLabel(profile.nextDueAt)}</span></td><td className="px-3 py-3 font-medium">{profile.ownerEmail || '—'}</td><td className="px-3 py-3"><select className="rounded border border-[#dedfdf] bg-white px-2 py-1.5 text-xs" value={planId} onChange={event => setPlanId(event.target.value)}><option value="">Custom / none</option>{plans.map(plan => <option key={plan.id} value={plan.id}>{plan.name}</option>)}</select></td><td className="px-3 py-3"><input className="w-20 rounded border border-[#dedfdf] px-2 py-1.5 font-mono text-xs" type="number" min="1" max="256" value={ipCount} onChange={event => setIpCount(event.target.value)} aria-label={`IP count for VM-${profile.vmid}`} /></td><td className="px-3 py-3"><input className="w-36 rounded border border-[#dedfdf] px-2 py-1.5 font-mono text-xs" type="number" min="0" placeholder="optional" value={customPrice} onChange={event => setCustomPrice(event.target.value)} aria-label={`Custom monthly price for VM-${profile.vmid}`} /></td><td className="px-3 py-3 font-mono">{money(profile.monthlyPriceCents, profile.currency)}</td><td className="px-5 py-3 text-right"><button type="button" onClick={() => void onSave(profile, planId, customPrice, ipCount)} disabled={saving !== null} className="rounded border border-[#1a1a1a] px-2.5 py-1.5 text-[11px] font-semibold hover:bg-[#f4f5f5] disabled:opacity-50">{saving === `vm:${profile.vmid}` ? 'Saving…' : 'Save'}</button></td></tr>;
};
