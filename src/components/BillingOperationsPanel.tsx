...
export const BillingOperationsPanel: React.FC = () => {
  ...
  const deleteServerCost = async (server: ApiBillingServerCost) => {
    const confirmed = window.confirm(`Delete the cost profile “${server.name}” for node ${server.nodeName}? This removes only the profitability configuration; VMs, invoices, payments, and Proxmox resources are not deleted.`);
    if (!confirmed) return;
    ...
    try {
      ...
      setServerCostForm(emptyServerCost);
      setNotice(`Cost profile deleted for ${server.nodeName}.`);
      await load();
