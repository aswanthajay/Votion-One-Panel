...
export const BillingOperationsPanel: React.FC = () => {
  ...
  const load = async () => {
    setLoading(true);
    setError(null);
    const sections = ['summary', 'pricing plans', 'invoices', 'billing policy', 'cost bases', 'dedicated-server costs', 'VM billing assignments', 'suspension actions'] as const;
    const requests = [
    ...
    const [summaryResult, plansResult, invoicesResult, configResult, costBasesResult, serverCostsResult, profilesResult, actionsResult] = results;
  ...
  const saveServerCost = async (event: React.FormEvent) => {
    ...
    try {
      await apiClient.upsertBillingServerCost({
        ...
        monthlyCostPaise: Number(serverCostForm.monthlyCostPaise),
        ipCostPaise: Number(serverCostForm.ipCostPaise),
  ...
  return (
    <main className="app-content min-h-full px-4 py-5 sm:px-6 lg:px-8" aria-busy={loading}>
      <div className="mx-auto w-full">
        ...
        <section className={`${cardClass} mt-6 overflow-hidden`}><div className="border-b border-[#dedfdf] px-5 py-4"><div className="flex items-start justify-between gap-4"><div><h2 className="text-sm font-semibold text-[#1a1a1a]">Dedicated-server cost model</h2><p className="mt-1 max-w-3xl text-xs leading-5 text-[#656b6b]">Configure the actual monthly price of each dedicated server and the recurring cost of additional public IPs. All inputs are in INR paise; metrics remain estimates until these values are entered.</p></div><span className="rounded border border-[#dedfdf] bg-[#f4f5f5] px-2 py-1 text-[10px] font-semibold uppercase text-[#656b6b]">INR cost basis</span></div></div><form onSubmit={saveServerCost} className="grid gap-3 border-b border-[#ededed] bg-[#fbfaf9] p-5 sm:grid-cols-2 lg:grid-cols-4"><input className={fieldClass} placeholder="Cost profile name" value={serverCostForm.name} onChange={event => setServerCostForm({ ...serverCostForm, name: event.target.value })} required /><input className={fieldClass} placeholder="Proxmox node name" value={serverCostForm.nodeName} onChange={event => setServerCostForm({ ...serverCostForm, nodeName: event.target.value })} required /><input className={fieldClass} type="number" min="0" placeholder="Server price (paise / month)" value={serverCostForm.monthlyCostPaise} onChange={event => setServerCostForm({ ...serverCostForm, monthlyCostPaise: event.target.value })} required /><input className={fieldClass} type="number" min="0" placeholder="Each extra IP (paise / month)" value={serverCostForm.ipCostPaise} onChange={event => setServerCostForm({ ...serverCostForm, ipCostPaise: event.target.value })} required /><input className={fieldClass} type="number" min="0" placeholder="Planned VM capacity" value={serverCostForm.plannedVmCapacity} onChange={event => setServerCostForm({ ...serverCostForm, plannedVmCapacity: event.target.value })} /><input className={fieldClass} type="number" min="0" placeholder="Included IPs" value={serverCostForm.includedIpCount} onChange={event => setServerCostForm({ ...serverCostForm, includedIpCount: event.target.value })} /><button type="submit" disabled={saving !== null} className="h-9 rounded bg-[#1a1a1a] px-4 text-xs font-semibold text-white hover:bg-[#333] disabled:opacity-50 lg:col-span-2">{saving === 'server-cost' ? 'Saving…' : serverCostForm.id ? 'Update server cost' : 'Add server cost'}</button></form>{serverCosts.length === 0 ? <div className="px-5 py-10 text-center text-sm text-[#8a9090]">No dedicated-server cost profiles configured. Add one profile for each Proxmox node to calculate infrastructure break-even and profit.</div> : <div className="overflow-x-auto"><table className="min-w-[980px] w-full text-left"><thead className="bg-[#fbfaf9] text-[10px] uppercase tracking-[0.12em] text-[#8a9090]"><tr><th className="px-5 py-3 font-semibold">Server / node</th><th className="px-3 py-3 font-semibold">Server cost</th><th className="px-3 py-3 font-semibold">IP economics</th><th className="px-3 py-3 font-semibold">VM capacity</th><th className="px-3 py-3 font-semibold">Utilization</th><th className="px-5 py-3 text-right font-semibold">Action</th></tr></thead><tbody className="divide-y divide-[#ededed]">{serverCosts.map(server => <tr key={server.id} className="text-xs text-[#1a1a1a]"><td className="px-5 py-3"><span className="font-semibold">{server.name}</span><span className="mt-1 block font-mono text-[10px] text-[#8a9090]">{server.nodeName}</span></td><td className="px-3 py-3 font-mono">{moneyPaise(server.monthlyCostPaise)}<span className="mt-1 block text-[10px] text-[#8a9090]">monthly base</span></td><td className="px-3 py-3 font-mono">{moneyPaise(server.ipCostPaise)}<span className="mt-1 block text-[10px] text-[#8a9090]">per IP · {server.includedIpCount} included</span></td><td className="px-3 py-3">{server.assignedVmCount} <span className="text-[#8a9090]">/ {server.plannedVmCapacity || '—'} VMs</span></td><td className="px-3 py-3">{server.assignedIpCount} IPs<span className="mt-1 block text-[10px] text-[#8a9090]">{Math.max(0, server.assignedIpCount - server.includedIpCount)} billable</span></td><td className="px-5 py-3 text-right"><button type="button" onClick={() => setServerCostForm({ id: server.id, name: server.name, nodeName: server.nodeName, monthlyCostPaise: String(server.monthlyCostPaise), ipCostPaise: String(server.ipCostPaise), plannedVmCapacity: String(server.plannedVmCapacity), includedIpCount: String(server.includedIpCount), isActive: server.isActive })} className="rounded border border-[#dedfdf] px-2.5 py-1.5 text-[11px] font-semibold hover:bg-[#f4f5f5]">Edit</button></td></tr>)}</tbody></table></div>}</section>
