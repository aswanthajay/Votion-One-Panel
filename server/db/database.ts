...
export class DatabaseService {
  ...
  async getVMs(ownerEmail?: string, vmid?: number, proxmoxConnectionId?: string) {
    ...
    const mapped = res.rows.map(v => ({
      ...
      type: v.type,
      node: v.node,
      displayNode: v.display_node || null,
      displayCpuModel: v.display_cpu_model || null,
      nodeDisplayName: v.display_node || v.proxmox_node_display_name || v.proxmox_connection_name || v.node,
